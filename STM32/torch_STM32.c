...
uint8_t SPI_transmit(SPI_HandleTypeDef *hspi, uint8_t *transmissionData_ptr, uint8_t transmissionDataLen)
{
	...
	while(attempts < 3) {
		status = HAL_SPI_Transmit(hspi, transmissionData_ptr, transmissionDataLen, HAL_MAX_DELAY);
...
uint8_t SPI_receive(SPI_HandleTypeDef *hspi, uint8_t *receptionData_ptr, uint8_t receptionDataLen)
{
	HAL_StatusTypeDef status;													// status indicates whether the reception was successful or not
	uint8_t attempts = 0;														// reception attempts (set to 3)
	...
	while(attempts < 3) {
		status = HAL_SPI_TransmitReceive(hspi, dummies, receptionData_ptr, receptionDataLen, HAL_MAX_DELAY);
