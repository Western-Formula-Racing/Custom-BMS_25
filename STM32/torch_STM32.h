...
uint8_t SPI_transmit(SPI_HandleTypeDef *hspi,			// SPI peripheral (hspi1 to 3 for TORCH 2025)
					 uint8_t *transmissionData_ptr,		// Transmission data array
					 uint8_t transmissionDataLen		// Transmission data length
					 );
					 
uint8_t SPI_receive(SPI_HandleTypeDef *hspi,			// SPI peripheral (hspi1 to 3 for TORCH 2025)
