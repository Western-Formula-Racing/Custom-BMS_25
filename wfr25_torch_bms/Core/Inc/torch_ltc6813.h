...
extern uint16_t PEC15_table[256];		// LTC6813 CRC look up table
extern uint16_t CRC15_Poly;				// LTC6813 CRC polynomial


// Creates the CRC look up table
void setup_PEC15(void);

// Calculates the PEC15 code for CRC
...
// Appends the PEC15 code to the message that'll be sent to the LTC6813
...
// Validates the PEC15 code sent by the LTC6813
uint8_t verify_PEC15(uint8_t *receivedData_ptr
					);

// Command group that order the LTC6813 to perform an action
void action_cmd(uint8_t *cmd_ptr,		// Pointer to the command
				uint8_t sideA			// Side (which LTC6813)
			   );

// Command group that writes data to the LTC6813 register(s)
void write_cmd(uint8_t *cmd_ptr,		// Pointer to the command
			   uint8_t *payload_ptr,	// Pointer to the data that's going to be sent
			   uint8_t sideA			// Side (which LTC6813)
			   );

// Command group that reads data back from the LTC6813
void read_cmd(uint8_t *cmd_ptr,					// Pointer to the command
			  uint8_t *receivedPayload_ptr,		// Pointer to the data that's going to be received
			  uint8_t sideA						// Side (which LTC6813)
			  );

// Forces both LTC6813s to enter the REFUP state
void force_refup(void);

// Checks if both LTC6813s have entered the REFUP state (returns 1 if they're both in REFUP, 0 otherwise)
uint8_t refup_check(void);

// Forces both LTC6813s to be muted (prevents cell balancing)
void force_mute(void);

// Forces both LTC6813s to be unmuted (permits cell balancing)
